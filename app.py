...
with st.sidebar:
    ...
    with st.expander("💬 Slack에 알림"):
        slack_webhook = st.text_input("Slack Webhook URL", type="password",
                                      help="Slack 앱 → Incoming Webhooks 발급. 비우면 .env(SLACK_WEBHOOK)")
...
audio_files = st.file_uploader("🎙️ 또는 음성 파일 (mp3·wav·m4a — Gemini가 자동 전사)",
                               type=["mp3", "wav", "m4a", "ogg", "flac", "aac"],
                               accept_multiple_files=True, key="audio")
...
def post_to_slack(webhook, text):
    """Slack Incoming Webhook으로 메시지 전송 (stdlib만 사용 — 의존성 X)"""
...
def transcribe_audio(client, f):
    """오디오 파일 → Gemini로 텍스트 전사 (Files API: 큰 파일·다양한 포맷 대응)"""
    ...
    r = client.models.generate_content(
        model="gemini-2.5-flash",
        contents=[
            "이 오디오는 강의/회의 녹음이다. 들리는 내용을 빠짐없이 한국어로 정확히 받아쓰기(전사)하라. "
            "요약·해설·화자 표기 없이, 말한 내용 그대로의 텍스트만 출력하라.",
            uploaded,
        ],
    )
    try:
...
if st.button("정리하기", type="primary"):
    if not lecture.strip() and not audio_files:
        st.warning("강의 자료(파일·텍스트·음성)를 올려줘.")
    else:
        try:
            client = genai.Client(api_key=api_key) if api_key.strip() else genai.Client()
            if audio_files:                                   # 🎙️ 음성 → 텍스트 먼저 변환
                with st.status("🎙️ 음성 → 텍스트 변환 중... (길면 1~2분)", expanded=True) as astat:
                    trans = []
                    for af in audio_files:
                        astat.write(f"변환 중: {af.name}")
                        trans.append(f"### [{af.name} 전사]\n{transcribe_audio(client, af)}")
                    astat.update(label="🎙️ 음성 변환 완료", state="complete")
                lecture = (lecture + "\n\n" + "\n\n".join(trans)).strip()
            if quality_mode:
            ...
            hook = (slack_webhook.strip() or os.environ.get("SLACK_WEBHOOK") or "").strip()
            if hook:                                           # 💬 Slack 알림 전송
                try:
                    s_title = next((l.strip()[2:] for l in summary.split("\n")
                                    if l.strip().startswith("# ")), "강의 정리")[:100]
                    snippet = " ".join(summary.strip().split())[:280]
                    msg = f":white_check_mark: *강의 정리 완료* — {s_title}\n{snippet}…"
                    if notion_url:
                        msg += f"\n:page_facing_up: 노션: {notion_url}"
                    post_to_slack(hook, msg)
                    st.success("Slack에 알림 전송됨!")
                except Exception as se:
                    st.warning(f"Slack 전송 실패: {se}")
